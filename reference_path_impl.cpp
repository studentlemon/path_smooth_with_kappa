...
void ReferencePathImpl::setOriginalSpline(const tk::spline &x_s,
                                          const tk::spline &y_s,
                                          double max_s) {
    *original_x_s_ = x_s;
    *original_y_s_ = y_s;
...
void ReferencePathImpl::logBoundsInfo() const {
    for (const auto &bound : bounds_) {
        ...
        std::cout << "front: (" << front.x << ", " << front.y << "), bound: (" << front.lb << ", " << front.ub << ").";
        std::cout << "rear: (" << rear.x << ", " << rear.y << "), bound: (" << rear.lb << ", " << rear.ub << ").";
    }
}
...
//            std::cout << "Path is blocked at s: " << state.s;
...
//std::vector<double> ReferencePathImpl::getClearanceWithDirectionStrict(const State &state)
//{
...
//    } else {
//        std::cout << "ref state is close to obstacles! xy: " << state.x << ", " << state.y;
//        return {0.0, 0.0};
//    }
...
//    double FLAGS_safety_margin = 1000.0;
...
bool ReferencePathImpl::buildReferenceFromSpline(double delta_s_smaller, double delta_s_larger) {
    ...
    if(delta_s_smaller> delta_s_larger)
        std::cout<<"less error"<<std::endl;

    if (isEqual(max_s_, 0.0)) {
        std::cout << "ref length is zero.";
        return false;
    }
    reference_states_.clear();
    ...
    while (tmp_s <= max_s_) {
        ...
        bool FLAGS_enable_dynamic_segmentation = true;
