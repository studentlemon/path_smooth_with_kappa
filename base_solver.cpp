...
bool BaseSolver::solve(std::vector<State> *optimized_path) {
...
void BaseSolver::setConstraints(Eigen::SparseMatrix<double> *matrix_constraints,
    ...
    double  FLAGS_max_steering_angle = 35.0 * M_PI / 180.0;
    double   FLAGS_wheel_base= 2.7;
    double FLAGS_expected_safety_margin = 0.6;
    ...
    *upper_bound = Eigen::MatrixXd::Zero(cons_size_, 1);
    ...
    bool FLAGS_constraint_end_heading = true;
...
void BaseSolver::getOptimizedPath(const Eigen::VectorXd &optimization_result,
    ...
    if(optimization_result.size()!= vars_size_)
